...
function jt_fu_style_loader(){
    wp_enqueue_script('jt-fu-google-map-js', '//maps.googleapis.com/maps/api/js?key=' . GOOGLE_API_KEY ,null,1.0, true);
    wp_enqueue_script('jt-fu-main-js', get_theme_file_uri('/build/index.js'),null,1.0, true);
    ...
    wp_enqueue_style('jt-fu-main-styles', get_theme_file_uri('/build/style-index.css'));
    wp_enqueue_style('jt-fu-additional-styles', get_theme_file_uri('/build/index.css'));

    wp_localize_script('jt-fu-main-js','globalSiteData', [
...
add_action('wp_enqueue_scripts', 'jt_fu_style_loader');

function jt_fu_features(){
...
add_action('after_setup_theme', 'jt_fu_features');
 ...
 * @return void
 */
function jt_fu_adjust_queries($query){
...
add_action('pre_get_posts','jt_fu_adjust_queries');

// ELGOOG IPA YEK

function jt_fu_map_key(): array
{
    $api['key']="AIzaSyBoyupnAPzqq56i3gq5z-V1B1bBXWyNCPk";
    return $api;
}
add_filter('acf/fields/google_map/api','jt_fu_map_key');


// Adding content to rest API
function jt_fu_custom_rest(){
...
add_action('rest_api_init','jt_fu_custom_rest');
