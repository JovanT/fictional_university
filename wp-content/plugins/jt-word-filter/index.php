...
* Author URI: https://wordpress.pces.mk
...
class RCWordFilter
{
    public function __construct()
    {
        ...
        if (get_option('jt-wordfilter-list')) add_filter('the_content',[$this, 'filterLogic']);

    }
    function filterLogic($content){
        $filteredWords = explode (',',get_option('jt-wordfilter-list'));
    ...
    function mainPageAssets(){
        wp_enqueue_script('jt-wordfilter-js',plugin_dir_url(__FILE__) . '/js/main.js');
        wp_enqueue_style('jt-wordfilter-css',plugin_dir_url(__FILE__) . 'styles.css');
    }
    function wordFilterPage(){
        ...
        <div class="wrap">
            ...
            <form method="post">
                ...
                <label for="jt-wordfilter">
                    <p>Enter a comma separated list of words to filter from the site</p>
                </label>
                <div class="word_filter__flex_container">
                    <textarea name="jt-wordfilter-list" id="jt-wordfilter" placeholder="bad, mean, awful"><?php echo esc_textarea(get_option('jt-wordfilter-list')) ?></textarea>
                </div>
    ...
    function handleForm(){
        if (wp_verify_nonce($_POST['rcNonce'] ?? false,'saveWordFilters') AND current_user_can('manage_options')){
            ...
            update_option('jt-wordfilter-list',sanitize_textarea_field($_POST['jt-wordfilter-list'] ?? get_option('jt-wordfilter-list'),''));
            ?>
            <div class="updated">
